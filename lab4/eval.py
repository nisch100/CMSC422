...
def ttest(Y, P0, P1, restrictTo=None):
    ...
    diff = np.sum(np.square(ahat - bhat))   # TODO
 
    # make sure it's not infinite
    diff = min(diff, 1e10)
 
    # compute the t-statistic
    t = (mu_a - mu_b) * np.sqrt((N*(N-1)) / diff) # TODO
...
#Y = loadFile('../lab4/prediction.truth')
#allP = [loadFile('../lab1/output/sentiment.de.pred-' + str(i)) for i in range(1, 21)]
 
#for i in range(0, 19):
#    print ttest(Y, allP[i], allP[19])

Y = loadFile('prediction.truth')
allP = [loadFile('output/sentiment.de.pred-' + str(i)) for i in range(1,21)]
print len(Y)
print len(allP[19])

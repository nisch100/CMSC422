...
class AlwaysPredictMostFrequent(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        '''
        self.mostFrequentClass = cmp(sum(Y), 0)
...
class FirstFeatureClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        ### TODO: YOUR CODE HERE
        for i,featVal in enumerate(X[:,0]):
            if featVal == 1:
                self.classForPos += Y[i]
            else:
                self.classForNeg += Y[i]

     
        self.classForPos = 1 if self.classForPos > 0 else -1
        self.classForNeg = 1 if self.classForNeg > 0 else -1

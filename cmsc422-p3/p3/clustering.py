...
def initialize_clusters(X, K, method):
    ...
    if method == 'determ':
        ...
        for k in range(1, K):
            # find m such that dzata point n is the best next mean, set
            # this to mu[k,:]
            ...
            for n in range(N):
                ...
                for k_prime in range(k):
                    norm = np.power(np.linalg.norm(X[n, :] - mu[k_prime, :]), 2)
                    if best_norm == -1 or norm < best_norm:
                        best_norm = norm

                if best_norm > bestDist:
                    bestN = n
                    bestDist = best_norm

            # set the mean
            mu[k, :] = X[bestN, :].copy()
            #debugging: print "mu(%d) = X(%d), distance = %g" % (k, bestN, bestDist)
